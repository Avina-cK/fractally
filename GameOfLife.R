...
for(i in 1:n0){ CS[i0x[i], i0y[i], 1]<-1 }
t<-1
...
neighbourhood<-function(M,x,y){
  ...
  #corners
  if(x==1 && y==1){ neighbours<-c(M[1,2], M[2,1], M[2,2] }  
  if(x==nrow(M) && y==ncol(M)){ neighbours<-c(M[x-1, y], M[x, y-1], M[x-1, y-1]) }
  if(x==nrow(M) && y==1){ neighbours<-c(M[x-1, y], M[x, y+1], M[x-1,y+1]) }
  if(x==1 && y==ncol(M)){ neighbours<-c(M[x, y-1], M[x+1, y], M[x+1, y-1]) }
  #1st row
  if(x==1 && y!=1 && y!=ncol(M)){ 
...
while(t<(T)){
  for(i in 1:nrow(CS[,,t])){
    for(j in 1:ncol(CS[,,t])){
      ...
      if(CS[i,j,t]==1){
        if(nt<2){ CS[i,j,t+1]<-0 }
        if(nt==2 || nt==3){ CS[i,j,t+1]<-1 }
        if(nt>3){ CS[i,j,t+1]<-0 }
      }
      if(CS[i,j,t]==0){ if(nt==3){ CS[i,j,t+1]<-1 } }
    }
  }
  t<-t+1
}
